...
bool Cmdl::parse(int argc_a, const char** argv_a) {
  ...
  char* debug_level_arg = opts_char_d[DEF_POS_DBGL];
  
  // check for empty string (if not, default level will remain set)
  //
  if (debug_level_arg[0] != '\0') {

    // assuming we have a legitimate option, then go ahead and try to set the
    // level. If there is a failure to do so, report to stdout and exit.
    //
    if (!debug_level_d.set_level(debug_level_arg)) {
    fprintf(stdout, "**> Cmdl::%s: unable to set \'-debug_level\' \'%s\'\n",
	    __FUNCTION__, debug_level_arg);
    return false;
    }
  };

  char* verbosity_level_arg = opts_char_d[DEF_POS_VRBL];
  
  // check for empty string (if not, default level will remain set)
  //
  if (verbosity_level_arg[0] != '\0') {

    // assuming we have a legitimate option, then go ahead and try to set the
    // level. If there is a failure to do so, report to stdout and exit.
    //
    if (!verbosity_level_d.set_level(verbosity_level_arg)) {
    fprintf(stdout, "**> Cmdl::%s: unable to set \'-debug_level\' \'%s\'\n",
	    __FUNCTION__, verbosity_level_arg);
    return false;
    }
  };
